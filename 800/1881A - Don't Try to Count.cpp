...
bool check(string s, string x)
{
    if(x.size() < s.size())
        return false;
    for(int i=0; i<x.size()-s.size()+1; i++)   // Sliding Window Concept obviously since substring ka to wahi rakhke check karenge har baar
    {
...
int main() 
{
	...
	while(t--)
	{
	    ...
	    if(check(s, x0))        // 5 operations max from time complexity
	        ans=0;
	    else if(check(s, x1))    
	        ans=1;
	    else if(check(s, x2))    
